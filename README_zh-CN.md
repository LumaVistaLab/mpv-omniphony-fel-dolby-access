...
JOC QMF 路径的 577 样本延迟，并正确解码 OAMD 的增益、继承、位置、更新时间和
换段语义，同时让呈现级 trim/warp 元数据与实时对象坐标保持分离。配套的
Omniphony 补丁会跨帧保留元数据事件，在精确样本处应用每次更新，并在扬声器和
双耳路径中渲染对象增益渐变。DD+ 仍然是有损传输格式；这些
...
- OAMD 的增益/状态默认值、前一对象与前次更新继承、差分位置、序列中断和全部
  block update 均会被解析；呈现级 trim/warp 元数据仍会解析，但不会用于修改回放时
  的对象坐标。block 起点为
...
在线播放显式启用 `--flatten-editions=yes`，将不同清晰度的版本展开为视频轨道，
因此 yt-dlp 返回的所有 DASH 视频与音频格式都会出现在对应的轨道菜单中。
已选中的单流视频、音频格式
...
格式菜单优先显示 B 站接口提供的标称码率，避免已打开的分段流被解复用器的局部
探测码率覆盖；未提供标称码率时仍使用解复用器码率。Playback statistics 中的
实时码率继续根据实际播放数据计算，与格式菜单的标称码率含义不同。

视频格式菜单统一显示接口提供的帧率，并保留小数部分，例如 `29.97`、`59.933`
和 `59.94`。默认流与候选流采用相同来源，选中、切换后均保持不变；没有接口
帧率的文件才回退到解复用器帧率。播放仍按媒体时间戳进行。

启动器优先使用所选运行包内的 `tools/ytdl_hook.lua`，缺失时回退到
`distribution/tools/ytdl_hook.lua`，使轨道描述语法与播放器版本保持配套。
该 hook 修正 B 站 DASH 元数据中的编码名称：
